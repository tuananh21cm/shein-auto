/**
 * shopAllocate — phân bổ sản phẩm win cho shop theo ngách (round-robin, chống trùng).
 * Đọc shop_niche → với mỗi ngách, lấy sp win (research_product, win>=70) → chia round-robin cho
 * các shop cùng ngách → ghi vào bảng shop_allocation (PRIMARY KEY goods_id = 1 sp chỉ 1 shop).
 *
...
const main = () => {
  ...
  const alreadyAllocated = new Set((db.prepare("SELECT goods_id FROM shop_allocation").all() as any[]).map((r) => r.goods_id));
  ...
  const tx = db.transaction(() => {
    for (const [niche, shops] of Object.entries(byNiche)) {
      ...
      const fresh = prods.filter((p) => !alreadyAllocated.has(p.goods_id));
      const counts: Record<string, number> = {};
      for (const s of shops) {
        counts[s] = (db.prepare("SELECT COUNT(*) n FROM shop_allocation WHERE shop=? AND niche_key=?").get(s, niche) as any).n;
      }

      let si = 0, added: Record<string, number> = {};
      for (const p of fresh) {
        let tries = 0;
        while (tries < shops.length && counts[shops[si]] >= target) { si = (si + 1) % shops.length; tries++; }
        if (tries >= shops.length) break; // mọi shop đã đủ target
        const shop = shops[si];
        insert.run({ ...p, shop, niche_key: niche, now: Date.now() });
        alreadyAllocated.add(p.goods_id);
        counts[shop]++; added[shop] = (added[shop] || 0) + 1;
        si = (si + 1) % shops.length;
      }
      ...
      for (const s of shops) {
        report.push({
          ...
          note: prods.length < target * shops.length ? `⚠️ ngách chỉ ${prods.length} sp win — thiếu để đủ ${target}/shop, cần cron research thêm` : "đủ hàng",
        });
      }
    }
  });
  ...
  db.close();
  out({ ok: true, target, allocations: report });
};
