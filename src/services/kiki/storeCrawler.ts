...
export async function crawlStore(page: Page, opts: CrawlStoreOptions = {}): Promise<StoreProduct[]> {
  ...
  try {
    ...
    while (map.size < maxProducts && stagnant < 5 && rounds < maxRounds) {
      rounds++;
      // SHEIN throttle cả phiên khi cuộn/gọi API quá nhiều → /risk/action/limit (403). Dừng NGAY,
      // đừng cuộn tiếp vào trang chặn (caller sẽ bỏ proxy này).
      if (/risk\/(action\/limit|challenge)/i.test(page.url())) {
        log(`⛔ risk/action/limit (403) — IP bị SHEIN throttle, dừng cào (thu được ${map.size}).`);
        break;
      }
      await ensureNoCaptcha(page, opts.captcha);
