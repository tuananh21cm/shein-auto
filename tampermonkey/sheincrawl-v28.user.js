// ==UserScript==
// @name         SHEIN Scraper v28 - Detail + Link Harvest
// @namespace    http://tampermonkey.net/
// @version      28.0.0
// @description  Cào SHEIN → POST thẳng lên shein-auto worker. v28: thêm GOM LINK từ trang listing/category/search → đẩy vào uncrawled queue (shop_allocation) cho crawlAllocated cào detail sau. Sync profile từ server. Realtime SSE. Detect out-of-stock per (color × size). Background tab vẫn cào nhờ silent audio.
// @author       shein-auto
// @match        *://*.shein.com/*
// @match        *://*.shein.co.uk/*
// @match        *://*.shein.de/*
...
(function () {
    ...
    // v28: đẩy 1 LÔ link sp (từ trang listing) vào uncrawled queue (shop_allocation) cho shop.
    async function apiIngestLinks(links, shops) {
        return gmRequest({
            url: `${SERVER}/admin/api/ingest/links`,
            method: 'POST',
            body: { links, shops },
        });
    }
    ...
    /* ============= v28: GOM LINK (Shift+click product card) ============= */
    // Dùng trên trang TÌM KIẾM / CATEGORY / LIST (CHƯA click vào detail).
    // User: giữ Shift + click product card → chọn (highlight). Chọn shop → ấn nút đẩy link vào uncrawled.
    const selectedLinks = new Map(); // goodsId -> { goodsId, url, name, image, price }

    // Từ element được click → tìm product card + trích link sp. null nếu không phải card.
    function extractCardFromEl(el) {
        if (!el || !el.closest) return null;
        let a = el.closest('a[href*="-p-"]');
        let card = el.closest('section, li, [class*="product-card"], [class*="S-product-item"], [class*="goods-"]');
        if (!a && card) a = card.querySelector('a[href*="-p-"]');
        if (!a) return null;
        const href = a.href || a.getAttribute('href') || '';
        const m = href.match(/-p-(\d+)\.html/);
        if (!m) return null;
        if (!card) card = a.closest('section, li') || a.parentElement;
        const img = (card || a).querySelector('img');
        const image = getOriginalImageUrl(img && (img.src || img.getAttribute('data-src') || (img.dataset && img.dataset.src)));
        const name = ((img && img.alt) || a.getAttribute('title') || a.textContent || '').trim().replace(/\s+/g, ' ').slice(0, 200);
        let price = null;
        const priceEl = (card || a).querySelector('[class*="price"]');
        if (priceEl) { const pn = parseFloat((priceEl.textContent || '').replace(/[^0-9.]/g, '')); if (!isNaN(pn)) price = pn; }
        return { goodsId: m[1], url: href.split('?')[0], name, image, price, _card: card || a };
    }

    function updateLinkCounter() {
        const c = document.getElementById('tm-link-count');
        if (c) c.innerText = selectedLinks.size;
        const btn = document.getElementById('tm-push-links');
        if (btn) btn.innerText = `📤 ĐẨY ${selectedLinks.size} LINK → UNCRAWLED`;
    }

    function toggleCardSelection(info) {
        const gid = info.goodsId;
        if (selectedLinks.has(gid)) {
            selectedLinks.delete(gid);
            if (info._card) info._card.classList.remove('tm-card-selected');
        } else {
            selectedLinks.set(gid, { goodsId: gid, url: info.url, name: info.name, image: info.image, price: info.price });
            if (info._card) info._card.classList.add('tm-card-selected');
        }
        updateLinkCounter();
    }

    // Re-apply highlight cho card đã chọn (SHEIN virtual-scroll re-render → mất class).
    function reapplyHighlights() {
        if (selectedLinks.size === 0) return;
        document.querySelectorAll('a[href*="-p-"]').forEach((a) => {
            const m = (a.href || '').match(/-p-(\d+)\.html/);
            if (!m || !selectedLinks.has(m[1])) return;
            const card = a.closest('section, li, [class*="product-card"], [class*="S-product-item"]') || a;
            if (card && !card.classList.contains('tm-card-selected')) card.classList.add('tm-card-selected');
        });
    }

    // Listener capture-phase: Shift+click product card → chọn, CHẶN điều hướng sang detail.
    let _linkSelectBound = false;
    function bindLinkSelect() {
        if (_linkSelectBound) return;
        _linkSelectBound = true;
        // mousedown: chặn sớm nav-bằng-mousedown + bôi đen text khi Shift (toggle vẫn ở click).
        document.addEventListener('mousedown', (e) => {
            if (!e.shiftKey) return;
            if (!extractCardFromEl(e.target)) return;
            e.preventDefault();
            e.stopPropagation();
        }, true);
        document.addEventListener('click', (e) => {
            if (!e.shiftKey) return;            // chỉ Shift+click mới là "chọn"
            const info = extractCardFromEl(e.target);
            if (!info) return;
            e.preventDefault();                 // không mở trang detail
            e.stopPropagation();
            toggleCardSelection(info);
        }, true);
    }

    async function pushSelectedLinks() {
        if (!API_TOKEN) { alert('Chưa có API token! Click ⚙ để thiết lập.'); return; }
        const selectedShops = Array.from(document.querySelectorAll('.tm-acc-checkbox:checked')).map((cb) => cb.value);
        if (selectedShops.length === 0) { alert('Chọn ít nhất 1 shop!'); return; }
        if (selectedLinks.size === 0) { alert('Chưa chọn card nào.\nGiữ Shift + click vào product card trên trang tìm kiếm để chọn.'); return; }

        const overlay = document.getElementById('tm-overlay');
        const overlayMsg = document.getElementById('tm-status-main');
        const status = document.getElementById('tm-status-text');
        const links = [...selectedLinks.values()];
        overlay.style.display = 'flex';
        overlayMsg.innerText = `ĐẨY ${links.length} LINK (${selectedShops.length} shop)...`;
        try {
            const r = await apiIngestLinks(links, selectedShops);
            const added = Object.values(r.perShop || {}).reduce((s, v) => s + (v.added || 0), 0);
            // Xoá selection + highlight
            document.querySelectorAll('.tm-card-selected').forEach((c) => c.classList.remove('tm-card-selected'));
            selectedLinks.clear();
            updateLinkCounter();
            status.innerText = `🔗 +${added} link vào uncrawled (gửi ${links.length})`;
            overlayMsg.innerText = `XONG! +${added} link → uncrawled`;
            setTimeout(() => { overlay.style.display = 'none'; }, 1800);
        } catch (e) {
            console.error('[SHEIN-SCRAPER] pushLinks', e);
            overlayMsg.innerText = '❌ LỖI: ' + e.message;
            status.innerText = 'FAIL: ' + e.message;
            setTimeout(() => { overlay.style.display = 'none'; }, 3500);
        }
    ...
    GM_addStyle(`
        ...
        .tm-card-selected{outline:3px solid #1d4ed8 !important;outline-offset:-3px;border-radius:8px;position:relative;}
        .tm-card-selected::after{content:'✓';position:absolute;top:6px;left:6px;background:#1d4ed8;color:#fff;font-size:13px;font-weight:800;width:22px;height:22px;line-height:22px;text-align:center;border-radius:50%;z-index:50;box-shadow:0 1px 4px rgba(0,0,0,.3);}
        #tm-link-harvest{border-top:1px dashed #ddd;margin-top:10px;padding-top:8px;}
        #tm-link-harvest .hint{font-size:11px;color:#666;text-align:center;line-height:1.5;}
        #tm-link-harvest .hint b{color:#1d4ed8;}
    `);

    function initUI() {
        reapplyHighlights();   // v28: giữ highlight card đã chọn khi SHEIN re-render list (virtual scroll)
        ...
        panel.innerHTML = `
            ...
            <div id="tm-link-harvest">
                <div class="hint">Trang tìm kiếm: giữ <b>Shift</b> + click product card để chọn<br>Đã chọn: <b id="tm-link-count">0</b> card</div>
                <button class="tm-btn-pro" id="tm-push-links" style="background:#1d4ed8;">📤 ĐẨY 0 LINK → UNCRAWLED</button>
            </div>
        ...
        document.getElementById('tm-push-links').onclick = pushSelectedLinks;
        bindLinkSelect();      // v28: bật Shift+click chọn card
        updateLinkCounter();
